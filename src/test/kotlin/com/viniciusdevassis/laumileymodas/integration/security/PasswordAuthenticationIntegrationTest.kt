...
class PasswordAuthenticationIntegrationTest:Phase4IntegrationTest(){
	@Test fun `cadastro publico sempre cria client com bcrypt e consentimento negado`() { val email=register();val row=jdbc.queryForMap("select a.role,a.password_hash,c.proactive_contact_authorized from account a join customer c on c.account_id=a.id where a.normalized_email=?",email);assertThat(row["role"]).isEqualTo("CLIENT");assertThat(row["password_hash"].toString()).startsWith("{bcrypt}");assertThat(row["proactive_contact_authorized"]).isEqualTo(false) }
