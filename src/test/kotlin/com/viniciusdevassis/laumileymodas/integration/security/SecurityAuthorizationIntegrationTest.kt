...
class SecurityAuthorizationIntegrationTest:Phase4IntegrationTest(){
	...
	@Test fun `admin Google autorizado recebe papel administrativo`() { val handoff=google.execute(VerifiedGoogleIdentity("test-admin-sub","admin@example.test",null,null));val token=googleExchange.execute(handoff.rawHandle).accessToken;mockMvc.get("/auth/me"){header(HttpHeaders.AUTHORIZATION,"Bearer $token")}.andExpect{status{isOk()};jsonPath("$.role"){value("ADMIN")}} }
