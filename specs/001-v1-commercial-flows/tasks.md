...
- [ ] T002 [P] Configurar datasource, Flyway, `ddl-auto=validate`, CORS, JWT, Google OAuth2, Cloudinary, WhatsApp e limites de upload em `src/main/resources/application.yaml`
...
- [ ] T006 Criar a migration de contas, clientes, identidades Google, consentimento e refresh tokens em `src/main/resources/db/migration/V1__create_identity_customers_and_tokens.sql`
...
- [ ] T027 [P] [US2] Escrever testes unitários de claims, assinatura, audiência, tipo e expiração dos JWT de access, refresh e registration em `src/test/kotlin/com/viniciusdevassis/laumileymodas/unit/infrastructure/security/JwtTokenServiceTest.kt`
- [ ] T028 [P] [US2] Escrever testes de integração da rotação, logout, expiração e detecção de reuso da família de refresh tokens em `src/test/kotlin/com/viniciusdevassis/laumileymodas/integration/security/RefreshTokenIntegrationTest.kt`
- [ ] T029 [P] [US2] Escrever testes de cadastro tradicional, senha codificada, login genérico e consentimento inicialmente negado em `src/test/kotlin/com/viniciusdevassis/laumileymodas/integration/security/PasswordAuthenticationIntegrationTest.kt`
- [ ] T030 [P] [US2] Escrever testes do login/cadastro Google, vínculo por `sub`, coleta complementar e restrição da única ADMIN em `src/test/kotlin/com/viniciusdevassis/laumileymodas/integration/security/GoogleAuthenticationIntegrationTest.kt`
- [ ] T031 [P] [US2] Escrever testes da matriz anônimo/CLIENT/ADMIN, JWT ausente, expirado e adulterado em `src/test/kotlin/com/viniciusdevassis/laumileymodas/integration/security/SecurityAuthorizationIntegrationTest.kt`
...
- [ ] T034 [P] [US2] Escrever testes HTTP de autenticação, confirmação de interesse e recuperação do link WhatsApp conforme OpenAPI em `src/test/kotlin/com/viniciusdevassis/laumileymodas/integration/http/AuthenticationAndInterestControllerIntegrationTest.kt`
...
- [ ] T038 [P] [US2] Implementar estados e transições do refresh token rotativo em `src/main/kotlin/com/viniciusdevassis/laumileymodas/domain/account/RefreshToken.kt`
...
- [ ] T047 [US2] Implementar entidade e adapter de refresh tokens com lock de rotação e busca por hash do `jti` em `src/main/kotlin/com/viniciusdevassis/laumileymodas/infrastructure/persistence/account/RefreshTokenJpaEntity.kt` e `src/main/kotlin/com/viniciusdevassis/laumileymodas/infrastructure/persistence/account/JpaRefreshTokenRepository.kt`
...
- [ ] T054 [US2] Implementar autenticação Google, registration token e restrição por `sub`/e-mail da ADMIN em `src/main/kotlin/com/viniciusdevassis/laumileymodas/application/auth/CompleteGoogleLoginUseCase.kt`, `src/main/kotlin/com/viniciusdevassis/laumileymodas/application/auth/CompleteGoogleCustomerRegistrationUseCase.kt` e `src/main/kotlin/com/viniciusdevassis/laumileymodas/infrastructure/security/oauth/GoogleIdentityVerifier.kt`
- [ ] T055 [US2] Implementar requests, responses e endpoints de cadastro, login, Google, refresh, logout e conta atual em `src/main/kotlin/com/viniciusdevassis/laumileymodas/presentation/auth/AuthRequests.kt`, `src/main/kotlin/com/viniciusdevassis/laumileymodas/presentation/auth/AuthResponses.kt` e `src/main/kotlin/com/viniciusdevassis/laumileymodas/presentation/auth/AuthController.kt`
- [ ] T056 [US2] Configurar API stateless, Resource Server JWT, OAuth2 Client, CORS, rotas públicas e autorização por papel em `src/main/kotlin/com/viniciusdevassis/laumileymodas/infrastructure/security/SecurityConfiguration.kt`
