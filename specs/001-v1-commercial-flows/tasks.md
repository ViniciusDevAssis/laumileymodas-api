...
- [ ] T029 [P] [US2] Escrever testes de cadastro tradicional sempre CLIENT, senha BCrypt, login válido/inválido e consentimento inicialmente negado em `src/test/kotlin/com/viniciusdevassis/laumileymodas/integration/security/PasswordAuthenticationIntegrationTest.kt`
...
- [ ] T056 [US2] Configurar API stateless com `@EnableMethodSecurity`, Resource Server JWT, OAuth2 Client, `CookieCsrfTokenRepository`, CORS sem curinga, rotas públicas, integração dos handlers 401/403 de T012 e autorização por papel em `src/main/kotlin/com/viniciusdevassis/laumileymodas/infrastructure/security/SecurityConfiguration.kt`
